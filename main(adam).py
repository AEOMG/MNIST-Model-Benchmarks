...
class SimpleNN:
    ...
    def update_parameters(self, dW1, db1, dW2, db2):
        ...
        # 매개변수 업데이트
        self.W1 -= lr_t * self.mW1 / (np.sqrt(self.vW1) + self.epsilon)
        self.b1 -= lr_t * self.mb1 / (np.sqrt(self.vb1) + self.epsilon)
        self.W2 -= lr_t * self.mW2 / (np.sqrt(self.vW2) + self.epsilon)
        self.b2 -= lr_t * self.mb2 / (np.sqrt(self.vb2) + self.epsilon)
